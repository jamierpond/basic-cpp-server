...
> struct StringEncoder {
  ...
  constexpr static auto decode(const std::string_view &str) {
    ...
    T result = reverse_encoding<CharacterSet, T, check_overflow, check_valid_chars>(str);
    return std::bit_cast<ToType>(result);
  }

  template <typename FromType = T>
  constexpr static auto encode_large_type(FromType value) {
      static_assert(sizeof(FromType) > sizeof(T));
      auto size = sizeof(FromType);
      auto v = std::bit_cast<FromType>(value);
      auto result = std::string{};
      result.reserve(size * 2);
      for (auto i = size - 1; i >= 0; --i) {
          auto byte = (v >> (i * 8)) & 0xFF;
          result += apply_encoding<CharacterSet, T>(byte);
      }
      return result;
  }
};
